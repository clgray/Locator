...
namespace Cnit.Testor.Core.UI
{
    ...
    {
        private static int _vp, _v5, _v4, _v3;
        ...
        private static string ResponsibleHuman = "Ответственный за тестирование (Ф.И.О)";
        private static string Sign = "Подпись";
        ...
        public static void PrintDataGridView(DataGridView dgv1, int vp, int v5, int v4, int v3)
        {
            ...
            try
            {
                ...
                switch (dlg.SelectedMode)
                {
                    case 1:
                        {
                            columnName = "pass";
                            dgv.Columns.Add(columnName, "Зачёт?");
                            foreach (DataGridViewRow row in dgv.Rows)
                            {
                                double current = (double)row.Cells["Score"].Value;
                                var cell = (row.Cells[columnName] as DataGridViewTextBoxCell);
                                if ((_vp == 0 && current >= (double)row.Cells["PassingScore"].Value) ||
                                    (_vp != 0 && current >= _vp))
                                    cell.Value = "зачёт";
                                else
                                    cell.Value = String.Empty;
                            }
                            SelectedColumns.Add("Зачёт?");
                            addColumns = true;
                        };
                        break;
                    case 2:
                        {
                            columnName = "mark";
                            dgv.Columns.Add(columnName, "Оценка");
                            foreach (DataGridViewRow row in dgv.Rows)
                            {
                                double current = (double)row.Cells["Score"].Value;
                                var cell = (row.Cells[columnName] as DataGridViewTextBoxCell);
                    ...
                    case 3:
                        {
                            ...
                            {
                                double current = (double)row.Cells["Score"].Value;
                                double max = (double)row.Cells["MaxScore"].Value;
                                double prs = 0;
                                if (max != 0)
                                    prs = ((double)current / max) * 100.0;
                                var cell = (row.Cells[columnName] as DataGridViewTextBoxCell);
                                if (prs != 0)
                                    cell.Value = prs.ToString("00.0") + "%";
                                else
                                    cell.Value = String.Empty;
                            }
                            SelectedColumns.Add("%");
                ...
                if (addColumns)
                    dgv.Columns[columnName].Width = 60;
                PrintAllRows = dlg.PrintAllRows;
                FitToPageWidth = dlg.FitToPageWidth;

                RowsPerPage = 0;
                ppvw = new PrintPreviewDialog();
                Form printForm = (ppvw as Form);
                printForm.Text = "Предварительный просмотр";
            ...
            try
            {
                ...
                if (PageNo == 1)
                {
                    foreach (DataGridViewColumn GridCol in dgv.Columns)
                    {
                        ...
                        if (!DataGridViewPrinter.SelectedColumns.Contains(GridCol.HeaderText)) continue;
                ...
                while (RowPos <= dgv.Rows.Count - 1)
                {
                    ...
                    else
                    {
                        if (NewPage)
                        {
                            Font fnt = new Font(dgv.Font.FontFamily, dgv.Font.Size + 3);
                            e.Graphics.DrawString(ResponsibleHuman, new Font(fnt, FontStyle.Bold),
                                  Brushes.Black, e.MarginBounds.Left, e.MarginBounds.Top -
                            e.Graphics.MeasureString(ResponsibleHuman, new Font(fnt,
                                  FontStyle.Bold), e.MarginBounds.Width).Height - 23);
                            //--------------------------------------------------------------------------------
                            e.Graphics.DrawString(Sign, new Font(fnt, FontStyle.Bold),
                                  Brushes.Black, e.MarginBounds.Left, e.MarginBounds.Top -
                            e.Graphics.MeasureString(Sign, new Font(fnt,
                                  FontStyle.Bold), e.MarginBounds.Width).Height + 8);
                            //--------------------------------------------------------------------------------
                            ...
                            e.Graphics.DrawString(s, new Font(dgv.Font, FontStyle.Bold),
                                    ...
                                    FontStyle.Bold), e.MarginBounds.Width).Width), e.MarginBounds.Top + 4);
                            ...
                            {
                                if (!GridCol.Visible) continue;
                                if (!DataGridViewPrinter.SelectedColumns.Contains(GridCol.HeaderText))
                                    continue;
